...
# Load .env with a flexible lookup order so starting the server from other
# working directories still picks up the project's .env when present.
# Priority:
#  1. CWD/.env
#  2. package directory (where this file lives)/.env
#  3. package parent (project root)/.env
#  4. fallback to default load_dotenv behavior
def _flexible_load_dotenv():
    candidates = [
        _Path.cwd() / ".env",
...
# Perform flexible load at import time so environment variables from .env
# are available for subsequent initialization steps.
_flexible_load_dotenv()
...
class LightRAG:
    ...
    def __post_init__(self):
        ...
        # Optional: load ontology JSON from path specified in ONTOLOGY_PATH env var
        # If provided, attach loaded ontology to addon_params and, when appropriate,
        # use its "entity_types" and "relation_predicates" values.
        try:
            ...
            if ontology_path:
                ...
                _candidate = _Path(ontology_path)
                # If path is relative, try resolving relative to package folder
                if not _candidate.is_absolute():
                    _pkg_base = _Path(__file__).resolve().parent
                    _candidate_pkg = _pkg_base / ontology_path
                    if _candidate_pkg.exists():
                        _candidate = _candidate_pkg

                if _candidate.exists():
                    with _candidate.open("r", encoding="utf-8") as _f:
                        _ontology = _json.load(_f)
                    # Attach full ontology and relation predicates to addon_params for later use
                    # Normalize ontology schema so downstream code can rely on consistent fields
                    norm = {}

                    # Entity types: accept multiple shapes. New schema may include
                    # an `ontology` object with `nodeTypes`, or top-level `entity_types`.
                    def _normalize_type(t: str) -> str:
                        # Keep normalization consistent with entity extraction pipeline
                        return str(t).replace(" ", "").lower()

                    raw_entity_types = (
                        _ontology.get("entity_types")
                        or _ontology.get("nodeTypes")
                        or (_ontology.get("ontology") and _ontology.get("ontology").get("nodeTypes"))
                        or (_ontology.get("ontology") and _ontology.get("ontology").get("entity_types"))
                    )
                    if isinstance(raw_entity_types, dict):
                        norm["entity_types_map"] = raw_entity_types
                        norm["entity_types"] = list(raw_entity_types.keys())
                        norm["entity_types_normalized"] = [
                            _normalize_type(t) for t in norm["entity_types"]
                        ]
                    elif isinstance(raw_entity_types, list):
                        norm["entity_types_map"] = {t: "" for t in raw_entity_types}
                        norm["entity_types"] = raw_entity_types
                        norm["entity_types_normalized"] = [
                            _normalize_type(t) for t in raw_entity_types
                        ]
                    else:
                        norm["entity_types_map"] = {}
                        norm["entity_types"] = []
                        norm["entity_types_normalized"] = []

                    # Relation predicates: accept legacy list or the newer `edgeTypes` array
                    rel_preds = []
                    if isinstance(_ontology.get("relation_predicates"), list):
                        rel_preds = [str(p) for p in _ontology.get("relation_predicates")]
                    elif isinstance(_ontology.get("predicates"), list):
                        rel_preds = [str(p) for p in _ontology.get("predicates")]
                    elif isinstance(_ontology.get("edgeTypes"), list):
                        # edgeTypes is expected to be a list of objects with a `name` field
                        for et in _ontology.get("edgeTypes"):
                            if isinstance(et, dict) and et.get("name"):
                                rel_preds.append(str(et.get("name")))
                    norm["relation_predicates"] = rel_preds

                    # Allowed relations: produce mapping predicate -> {src_types:[], tgt_types:[]}
                    norm_allowed = {}
                    # Legacy allowed_relations or relation_schema
                    allowed = _ontology.get("allowed_relations") or _ontology.get("relation_schema")
                    if isinstance(allowed, dict):
                        for pred, rule in allowed.items():
                            if not isinstance(pred, str) or not isinstance(rule, dict):
                                continue
                            src_types = rule.get("src_types") or rule.get("source_types") or rule.get("src") or []
                            tgt_types = rule.get("tgt_types") or rule.get("target_types") or rule.get("tgt") or []
                            if isinstance(src_types, list) and isinstance(tgt_types, list):
                                norm_allowed[pred] = {
                                    "src_types": [str(x) for x in src_types],
                                    "tgt_types": [str(x) for x in tgt_types],
                                }

                    # New-style: derive allowed_relations from edgeTypes
                    if isinstance(_ontology.get("edgeTypes"), list):
                        for et in _ontology.get("edgeTypes"):
                            if not isinstance(et, dict):
                                continue
                            name = et.get("name") or et.get("predicate")
                            if not name:
                                continue
                            srcs = et.get("from") or et.get("src_types") or et.get("source_types") or []
                            tgts = et.get("to") or et.get("tgt_types") or et.get("target_types") or []
                            if isinstance(srcs, list) and isinstance(tgts, list):
                                # Normalize type names to match entity extraction storage
                                norm_allowed[name] = {
                                    "src_types": [_normalize_type(x) for x in srcs],
                                    "tgt_types": [_normalize_type(x) for x in tgts],
                                }

                    norm["allowed_relations"] = norm_allowed

                    # Preserve any other ontology fields under 'raw'
                    norm["raw"] = _ontology

                    # Only override entity_types from ontology if user didn't explicitly set ENTITY_TYPES
                    if norm.get("entity_types") and os.getenv("ENTITY_TYPES") is None:
                        self.addon_params["entity_types"] = norm.get("entity_types")

                    # Attach normalized ontology and relation_predicates to addon_params for later use
                    self.addon_params["ontology"] = norm
                    if norm.get("relation_predicates"):
                        self.addon_params["relation_predicates"] = norm.get(
                            "relation_predicates"
                        )
                    logger.info(f"Loaded ontology from {_candidate}")
                else:
                    logger.warning(f"ONTOLOGY_PATH set but file not found: {ontology_path}")
        except Exception as _e:
            logger.warning(f"Failed to load ontology from ONTOLOGY_PATH: {_e}")
